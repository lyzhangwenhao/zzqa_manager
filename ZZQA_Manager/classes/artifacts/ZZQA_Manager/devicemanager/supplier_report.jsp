...
<%
	...
	int nowpage=1;
	int pagerow=10;
	if (session.getAttribute("keywords_supplier") != null) {
		keywords = (String)session.getAttribute("keywords_supplier");
	}else{
		session.setAttribute("keywords_supplier", keywords);
	}
	if (session.getAttribute("nowpage_supplier") != null) {
		nowpage = (Integer)session.getAttribute("nowpage_supplier");
	}else{
		session.setAttribute("nowpage_supplier", nowpage);
	}
	if (session.getAttribute("pagerow_supplier") != null) {
		pagerow = (Integer)session.getAttribute("pagerow_supplier");
	}else{
		session.setAttribute("pagerow_supplier", pagerow);
	}
	List<Customer_data> customers=customer_dataManager.getCustomerByCondition(2,keywords,nowpage,pagerow);
	int num=customer_dataManager.getNumByCondition(2,keywords);
	int allpage=(num%pagerow==0?0:1)+num/pagerow;
	boolean permission70=permissionsManager.checkPermission(mUser.getPosition_id(), 70);
	...
	pageContext.setAttribute("permission70", permission70);
%>
...
<html>
	...
	-->
		...
		function showUserDialog(id){
			...
			if( id.length>0){
				...
				if($(".customer_top").length==0){
					$(".dialog_customer").prepend('<div class="customer_top"><span>供应商编码</span><span>'+customer_id+'</span></div>');
				}else{
		...
		function closeDialog(btn_id){
			if(btn_id==1){
				//删除
				initdiglogtwo2("提示信息","你确定要删除该供应商吗？");
		...
		function delCustomer(){
			$.ajax({
				type:"post",//post方法
				url:"DeviceServlet",
				data:{"type":"checkCanDelCustomer","customer_id":nowID,"customer_type":2},
				timeout : 15000, 
				//ajax成功的回调函数
				success:function(returnData){
					if(returnData==0){
						...
						document.operaform.customer_type.value=2;
		...
		function addCustomer(){
			...
			$.ajax({
				type:"post",//post方法
				url:"DeviceServlet",
				data:{"type":"checkcustomer","company_name":company_name,"customer_type":2},
				timeout : 15000, 
				//ajax成功的回调函数
				success:function(returnData){
					if(returnData==0){
		...
		<div class="td1_device">
			<form action="DeviceServlet?type=customerfilter" method="post"
				...
				<img title="添加" src="images/add_materials.png" id="addCustomer" 
						onclick="showUserDialog('');" class="addCustomer" <%if(!permission70){ %>style="position:relative;z-index:-1"<%} %>>
					<%if(permission70){ %>
					<img title="导入表格" src="images/import_materials.png" onclick="$('#exportExcel_div .jFiler-input').click();" class="importCustomer">
					<%} %>
					<img title="导出表格" src="images/export_track.png" id="img_export" onclick="loadDownCustomer(2);" class="exportCustomer">
				...
				<table class="device_tab">
					...
					<tr id="tr<c:out value="${customer.customer_id}"></c:out>" class="tab_tr2 tr_pointer" onclick="showUserDialog('<c:out value="${customer.customer_id}"></c:out>');">
