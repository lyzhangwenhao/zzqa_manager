...
$(function(){
	//先写点击，防止第一次触发点击事件
	$("#bts-ex-8 li.filter-item.items[data-value='"+sales_id+"']").click();
 ...
 */
function initTR(){
	$(".shipping_tr_content").each(function(i){
	...
	if(index==0){
		...
		if(nowTR){
			...
			if(nowTR.attr("product_id")==0){
				...
				if(contract_num.trim()==""||(contract_num!="0"&&contract_num!="0.0"&&!parseFloat(contract_num))){
					initdiglog2("提示信息","请输入合同数量");
					return;
				}
				if(unit_price.trim()==""||(unit_price!="0"&&unit_price!="0.0"&&!parseFloat(unit_price))){
					initdiglog2("提示信息","请输入单价");
					return;
				}
				if(last_num.trim()==""||(last_num!="0"&&last_num!="0.0"&&!parseFloat(last_num))){
			...
			var tr_num=$(".shipping_tr_content").length;
			var materials=getMaterialsByID(m_id);
			var price=num*unit_price;//本次发货金额
