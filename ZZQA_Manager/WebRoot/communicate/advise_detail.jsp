...
<%request.setCharacterEncoding("UTF-8");
	...
	List<Comments> commentsList=communicateManager.getCommentsListByCondition(1, advise_id, uid);
	boolean permission45=permissionsManager.checkPermission(mUser.getPosition_id(), 45);
	FormTransform ftf=new FormTransform();
	if(session.getAttribute("advise_index")==null){
		response.sendRedirect("../login.jsp");
		return;
	}
	int advise_index=(Integer)session.getAttribute("advise_index");
	communicateManager.updateRead_user(1, advise_id, uid);
%>
		...
		<script type="text/javascript">
			...
			function publish_comment(){
				...
				$.ajax({
					...
					data:{"type":"addComment","c_type":1,"foreign_id":<%=advise_id%>,"content":content},
					//ajax成功的回调函数
			...
			function getCommentStr(comments_id,content){
				...
				+(permission45?('<a href="javascript:addReplyToComment('+comments_id+')">回复</a><div></div>'):'')
			...
			function getReplyStr(comments_id,reply_id,content,reply_name){
				...
				+(permission45?('<a href="javascript:addReplyToReply('+comments_id+','+reply_id+')">回复</a><div></div>'):'')
				+'<a href="javascript:agree('+reply_id+',3,false,0)" id="reply_agree'+reply_id+'">赞同 </a>'
				+'</div>';
				return comm;
			}
			function updatecomment(id){
				var area=$(".advise_reply_areatext_div");
				if(area.length==0||area.prop("id")!=("updatecomment"+id)){//已打开的不变
					...
					var temp='<div class="advise_reply_areatext_div" id="updatecomment'+id+'"><textarea class="advise_reply_areatext" maxlength="2000" placeholder="回复 '+replyname+'：">'+transBRToRN(content)+'</textarea><div class="advise_reply_btn_div"><div onclick="updatecommentcontent('+id+')">确认修改</div><div onclick="clearReplyArea();">取消修改</div></div></div>';
					$("#comment_item"+id).append(temp);
					$(".advise_reply_areatext").focus();
				}
			}
			function updatecommentcontent(id){
				var content=$(".advise_reply_areatext").val();
				if(content.replace(/\s/g,"").length>0){
					$.ajax({
						type:"post",//post方法
						url:"CommunicateServlet",
			...
			function addReplyToComment(id){
				...
				if(area.length==0||area.prop("id")!=("addtocomment"+id)){//已打开的不变
					clearReplyArea();
					var replyname=$("#comment_item"+id).find(".comment_name").text();
					var temp='<div class="advise_reply_areatext_div" id="addtocomment'+id+'"><textarea class="advise_reply_areatext" maxlength="2000" placeholder="回复 '+replyname+'："></textarea><div class="advise_reply_btn_div"><div onclick="addReplyToCommentContent('+id+')">确认回复</div><div onclick="clearReplyArea();">取消回复</div></div></div>';
					$("#comment_item"+id).append(temp);
					$(".advise_reply_areatext").focus();
				}
			}
			...
			function updateReply(cid,rid){
				...
				if(area.length==0||area.prop("id")!=("updatereply"+rid)){//已打开的不变
					...
					var temp='<div class="advise_reply_areatext_div" id="updatereply'+rid+'"><textarea class="advise_reply_areatext" maxlength="2000" placeholder="回复 '+replyname+'：">'+transBRToRN(content)+'</textarea><div class="advise_reply_btn_div"><div onclick="updateReplyContent('+rid+')">确认修改</div><div onclick="clearReplyArea();">取消修改</div></div></div>';
			...
			function addReplyToReply(cid,rid){
				...
				if(area.length==0||area.prop("id")!=("addtoreply"+rid)){//已打开的不变
					...
					var temp='<div class="advise_reply_areatext_div" id="addtoreply'+rid+'"><textarea class="advise_reply_areatext" maxlength="2000" placeholder="回复 '+replyname+'："></textarea><div class="advise_reply_btn_div"><div onclick="addReplyToReplyContent('+cid+','+rid+')">确认回复</div><div onclick="clearReplyArea();">取消回复</div></div></div>';
					$("#comment_item"+cid).append(temp);
					$(".advise_reply_areatext").focus();
				}
			}
				...
				if(content.replace(/\s/g,"").length>0){
					$.ajax({
						...
						success:function(returnData){
							...
							clearReplyArea();
							var replyname=$("#reply_item"+rid).find(".reply_name span:eq(0)").text();
							//replyname=replyname.substring(0,replyname.indexOf(" "));
			...
			function clearReplyArea(){
				$(".advise_reply_areatext_div").remove();
			}
	...
	<jsp:param name="index" value="4" />
		...
		<div class="div_center">
			...
			<div class="div_center_right">
				<div class="del_advise_div_img" onclick="closeAdvise()"></div>
					<div class="div_title2">
					<%=advise.getTitle() %>
					</div>
					<div class="div_dashed1"></div>
					<div class="advise_detail_content">
					<%=ftf.transRNToBR(advise.getContent())%>
					</div>
					<div class="advise_detail_bottom_div">
						<div class="advise_detail_bottom">
						<div>
						<%=advise.getCreate_name() %>
						</div>
						<div>
						<%=advise.getUpdate_date() %>
						</div>
					</div>
					</div>
					<div class="advise_file_num">附件<%=filList.size()>0?("：（"+filList.size()+"个）"):"" %></div>
					...
					<%if(permission45){ %>
					<textarea class="advise_comment_areatext" id="comment_textarea" placeholder="畅所欲言" maxlength="2000"></textarea>
					<div class="advise_comment_btn_div"><div onclick="publish_comment()">评 论</div></div>
					<%} %>
						...
						<div class="comment_item" id="comment_item<%=comments.getId()%>">
							...
							<%for(Reply reply:comments.getReplyList()) {%>
								<div class="reply_item" id="reply_item<%=reply.getId()%>">
									...
									<div class="reply_handel" >
										<%if(uid==reply.getCreate_id()){ %><a href="javascript:updateReply(<%=comments.getId()%>,<%=reply.getId()%>)">编辑</a><div></div><a href="javascript:delReply(<%=reply.getId()%>)">删除</a><div></div><%}if(permission45){ %><a href="javascript:addReplyToReply(<%=comments.getId()%>,<%=reply.getId()%>)">回复</a><div></div><%} %><a href="javascript:agree(<%=reply.getId()%>,3,<%=reply.isIsagree()%>,<%=reply.getAgree()%>)" id="reply_agree<%=reply.getId()%>"><%=reply.getAgree()==0?"赞同":((reply.isIsagree()?"已赞同":"赞同")+"（+"+reply.getAgree()+"）")%> </a>
									</div>
								</div>
							<%} %>
							</div>
		...
		<form action="CommunicateServlet" name="jumpform" method="post" >
			...
			<input type="hidden" name="jumpType" value="<%=advise_index%>">
